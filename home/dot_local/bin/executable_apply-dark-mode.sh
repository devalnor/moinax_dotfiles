...
# 3a. KDE color scheme + GTK colors.css regeneration
...
# This MUST run before gsettings/dconf writes (step 3c) because the bounce
# overwrites dconf color-scheme with an intermediate wrong value.
...
# 3c. GTK/GNOME portal backend — monitors dconf (must be LAST so the portal
# sees the correct final value, not the intermediate bounce from step 3a)
